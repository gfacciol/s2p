...
void bdint_gen(float *x, int w, int h, accumulator_t *a, float percentile)
{
	...
	float *acc_v = (float*)xmalloc(w*h*sizeof*acc_v); // accumulator for values
	float *acc_n = (float*)xmalloc(w*h*sizeof*acc_n); // value count
	for (int i = 0; i < w*h; i++)
	if (rep[i] == i)
	{
...
int main(int c, char *v[])
{
	...
	char *filename_in   = c > 1 ? v[1] : (char*) "-";
	char *filename_out  = c > 2 ? v[2] : (char*) "-";
